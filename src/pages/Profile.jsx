...
import { User, Mail, MapPin, Shield, Star, Hash, CheckCircle, AlertOctagon } from "lucide-react";

export default function Profile() {
  const { user, userRole } = useAuth();

  console.log("Profile User Data:", user); // Debug address issue
  ...
  // Prefer user.address from DB, fallback to user.location if exists (some apps use that), or default
            ...
            <div className="pt-16 pb-8 px-8">
                ...
                <div className="flex justify-between items-start mb-6">
                    <div>
                     ...
                     <div className={`px-4 py-1.5 rounded-full text-sm font-semibold capitalize border ${
                         userRole === 'admin' ? 'bg-purple-100 border-purple-200 text-purple-600' : 
                         userRole === 'chef' ? 'bg-orange-100 border-orange-200 text-orange-600' : 
                ...
                <div className="grid gap-6 md:grid-cols-2">
                        ...
                        <div className="flex items-center gap-2">
                             {user?.status === 'fraud' ? (
                                <>
                                  ...
                                  <span className="text-red-500 font-bold capitalize">{user?.status || "Unknown"}</span>
                                </>
                             ) : (
                                <>
                                 <CheckCircle className="text-green-500" size={20}/>
                                 <span className="text-green-500 font-bold capitalize">{user?.status || "Active"}</span>
                                </>
                             )}
                        </div>
                    </div>
                    ...
                    {/* Chef ID (Only for Chefs) */}
                     {userRole === 'chef' && (
                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                            ...
                            <div className="font-mono text-xl text-[#F79A19] tracking-widest">{chefId}</div>
                        </div>
                    )}
                 </div>
                 ...
                 {userRole === 'user' && (
                    <div className="mt-8 pt-6 border-t border-gray-100">
                        <h4 className="text-lg font-semibold text-gray-800 mb-4">Want to become a Chef or Admin?</h4>
